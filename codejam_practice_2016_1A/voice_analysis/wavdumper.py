...
### read_ methods for each chunk. ###
    ...
    def read_data(self, length):
        ...
        if self.waveFormat==1:
            samples = length/self.bytesPerSample

            self.p('%s samples' % int(samples))
            self.p('%.3f seconds' % (float(samples)/self.sampleRate))
        self.skip(length)
    ...
    def read_fmt(self, length):
        ...
        if length>=16:
            ...
            self.bytesPerSample = float(channels)*(math.ceil(bitsPerSample/8.0))
        else:
    ...
    def read_smpl(self, length):
        """Sampler info chunk."""
        manufacturer, product, samplePeriod, midiUnityNote, midiPitchFraction, smpteFormat, \
        smpteOffset, sampleLoops, samplerData = struct.unpack(self.endian+'9i', self.read(9*4))
        count = 9*4 # read bytes

        self.p('Sampler Chunk')
        self.p('Manufacturer: %x' % manufacturer)
        self.p('Product: %x' % product)
        noteStr = '%s-%s' % (NOTES[midiUnityNote % 12], midiUnityNote/12)
        self.p('MIDI note (60=middle-C): %s (%s) +- %.1f%%' % \
            (midiUnityNote, noteStr, float(midiPitchFraction)/0x80000000)) #here#

        for i in range(sampleLoops):
            self.p('Sample loop')
            self.indent += 1
    ...
    def read_sampleloop(self, additional):
        ...
        id, loopType, start, end, fraction, playCount = struct.unpack(self.endian+'6i', self.read(6*4))
